...
For a project build with no explicit output path, `raz build` emits a native executable rather than Forge IR. Debug executables go to `target/debug/bin/<package>` (`.exe` on Windows), while `raz build --release` writes `target/release/bin/<package>`. Modular Forge package/module objects live under `target/<profile>/packages/`; `target/<profile>/obj/` is reserved for the legacy whole-project fallback scratch object. Static/shared libraries use `target/<profile>/lib/`, backend IR uses `target/<profile>/ir/`, and semantic module metadata uses `target/<profile>/modules/`. These artifact-category directories are **lazy**: Raz creates a category only when that command actually writes an artifact into it. A normal modular Forge executable build therefore materializes `bin/` and `packages/` and does **not** create an empty `obj/`; an LLVM executable build materializes `bin/`; and fallback Forge emission creates `obj/` only when it is genuinely needed. `raz check` needs no profile artifact tree and normally writes only incremental state under `target/cache/`. Supplying an explicit output continues to honor that output for compiler/bootstrap workflows, and `raz forge` remains the explicit way to request `.fir` output.
