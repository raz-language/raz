...
Browser WebAssembly is emitted as a callable browser module rather than a WASI command. The compiler keeps only reachable `raz_web` imports, omits `wasi_snapshot_preview1` and its WASI-only function types, and does not synthesize `_start`. Browser lowering also compacts the Raz function graph itself: only functions reachable from browser roots (plus compiler-owned runtime dependencies), reachable closure adapters, and reachable async pollers receive WebAssembly definitions. Unreachable build-time functions do not receive function types, function declarations, trap placeholders, or table entries. This keeps browser modules independent from a WASI compatibility shim while leaving ordinary `raz build --target wasm` command modules unchanged.
...
Release-generated runtime URLs are page-relative rather than origin-rooted. Nested routes such as `docs/guides/index.html` point back to the centralized `dist/assets/` directory with the required `../` prefix, and fingerprinted JavaScript loads its main and lazy WebAssembly modules relative to the shipped asset directory. The same `dist/` tree therefore works unchanged at an origin root or below a hosting subpath (for example a project-style static-host URL).
...
Release analysis can also enforce byte budgets directly from `raz.toml`. Budgets are checked against the finalized fingerprinted output—not compiler intermediates—so they are suitable for CI regression gates. Every field is optional and uses bytes:
...
The analysis report adds a `Budgets:` section with `OK` or `EXCEEDED` for each configured ceiling. If any configured ceiling is exceeded, Raz still writes and prints the complete report, then `raz build --release --analyze` exits unsuccessfully. `js`/`javascript`, `meta`/`metadata`, and `asset`/`assets` are accepted aliases.
...
Interactive web modules keep stable logical host ABI identifiers internally, but emission compacts the physical `raz_web` import table to the browser calls reachable from the application's web roots. A component that only updates an integer text node therefore imports only `dom_set_text_i64` instead of the complete DOM/events/storage/history/timer surface. The same reachability graph drives defined-function compaction: direct calls, first-class function references, closure adapters, async pollers, synthesized allocator/future dependencies, exports, and table slots are remapped to one dense browser-only index space. Unreachable HIR functions are absent rather than represented by placeholder bodies. Generated JavaScript host bindings are usage-driven as well: reactive bundles emit only bindings imported by `app.wasm`, while static-first bundles union the requirements of `app.wasm` and every lazy Wasm chunk before stripping unused host functions. Private build markers are removed before release fingerprinting and never appear in deployable output.

### Usage-driven reactive JavaScript runtime

For `main() -> Component` applications, Raz also prunes the generated reactive JavaScript runtime from the optimized MIR call graph reachable from `main`. Event delegation, indexed state-binding updates, History-API routing, the HTTP/resource pump, scoped-component subtree synchronization, and raw-JavaScript evaluation are independent capabilities. A plain component that only renders HTML/CSS therefore receives a direct initial DOM publish rather than carrying the keyed patcher or unrelated event/router/HTTP machinery. Feature combinations retain the union of the helper groups they actually require; allocation or entry-point analysis failure falls back conservatively to the complete runtime rather than risking missing behavior.
