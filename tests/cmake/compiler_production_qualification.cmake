...
if(NOT release_configure_result EQUAL 0)
  message(FATAL_ERROR "production compiler qualification could not configure the optimized Forge toolchain:\n${release_configure_output}\n${release_configure_error}")
endif()
execute_process(
  COMMAND "${CMAKE_COMMAND}" --build --preset release --target raz-compiler-forge-codegen
  WORKING_DIRECTORY "${SOURCE_ROOT}"
...
if(NOT release_build_result EQUAL 0)
  message(FATAL_ERROR "production compiler qualification could not build the optimized Forge codegen helper:\n${release_build_output}\n${release_build_error}")
endif()
if(WIN32)
  set(optimized_forge_codegen "${SOURCE_ROOT}/build/release/tests/raz-compiler-forge-codegen.exe")
else()
  set(optimized_forge_codegen "${SOURCE_ROOT}/build/release/tests/raz-compiler-forge-codegen")
endif()
...
if(NOT candidate_build_result EQUAL 0)
  message(FATAL_ERROR "production compiler qualification could not build compiler candidate:\n${candidate_build_output}\n${candidate_build_error}")
endif()
...
execute_process(
  COMMAND "${candidate_executable}" --forge-native "compiler-package.txt" "compiler-output.fir"
  WORKING_DIRECTORY "${project}"
  RESULT_VARIABLE candidate_run_result)
if(NOT candidate_run_result EQUAL 0)
  message(FATAL_ERROR "compiler candidate returned ${candidate_run_result} while emitting production compiler")
endif()

set(production_ir "${project}/compiler-output.fir")
if(NOT EXISTS "${production_ir}")
  message(FATAL_ERROR "compiler candidate did not emit production compiler Forge IR")
endif()
file(SIZE "${production_ir}" production_ir_size)
if(production_ir_size LESS 500000)
  message(FATAL_ERROR "production compiler Forge module is unexpectedly small: ${production_ir_size} bytes")
endif()

if(NOT EXISTS "${production_object}")
  message(FATAL_ERROR "compiler candidate in-process Forge backend did not emit native production compiler object")
endif()
if(WIN32)
  execute_process(
...
if(NOT production_link_result EQUAL 0 OR NOT EXISTS "${production_executable}")
  message(FATAL_ERROR "Native production compiler link failed:\n${production_link_output}\n${production_link_error}")
endif()
...
# contains the compiler candidate arena declarations/calls.
...
execute_process(
  COMMAND "${production_executable}" --forge-native --opt=3 "main.rz" "structured-scalar.fir"
...
# introduce compiler candidate arena symbols and fail this link. `fabs` is supplied by the
...
execute_process(
  COMMAND "${production_executable}" --forge-native --opt=3 "main.rz" "structured-float.fir"
...
execute_process(
  COMMAND "${production_executable}" --forge-native --forge-structured-only --opt=3 "main.rz" "structured-aggregate.fir"
...
execute_process(
  COMMAND "${production_executable}" --forge-native --forge-structured-only --opt=3 "main.rz" "structured-array.fir"
...
execute_process(
  COMMAND "${production_executable}" --forge-native --forge-structured-only --opt=3 "main.rz" "structured-slice.fir"
...
execute_process(
  COMMAND "${production_executable}" --forge-native --forge-structured-only --opt=3 "main.rz" "structured-nested.fir"
...
execute_process(
  COMMAND "${production_executable}" --forge-native --forge-structured-only --opt=3 "main.rz" "structured-dynamic-array.fir"
...
execute_process(
  COMMAND "${production_executable}" --forge-native --forge-structured-only --opt=3 "main.rz" "structured-escape.fir"
...
execute_process(
  COMMAND "${production_executable}" --forge-native --forge-structured-only --opt=3 "main.rz" "structured-tls.fir"
...
file(WRITE "${smoke}/compiler-package.txt" "main.rz\n")
execute_process(
  COMMAND "${production_executable}"
  WORKING_DIRECTORY "${smoke}"
  RESULT_VARIABLE production_smoke_result)
if(NOT production_smoke_result EQUAL 0)
  message(FATAL_ERROR "Native production compiler compiler returned ${production_smoke_result} for the smoke package")
endif()

set(smoke_ir "${smoke}/compiler-output.fir")
if(NOT EXISTS "${smoke_ir}")
  message(FATAL_ERROR "Native production compiler compiler did not emit smoke Forge IR")
endif()

# The production compiler also supports explicit manifest/output paths. Keep
...
execute_process(
  COMMAND "${production_executable}" "compiler-package.txt" "explicit-output.fir"
  WORKING_DIRECTORY "${smoke}"
  RESULT_VARIABLE production_cli_result)
if(NOT production_cli_result EQUAL 0 OR NOT EXISTS "${cli_smoke_ir}")
  message(FATAL_ERROR "Native production compiler explicit-path CLI failed with ${production_cli_result}")
endif()
execute_process(
  COMMAND "${CMAKE_COMMAND}" -E compare_files "${smoke_ir}" "${cli_smoke_ir}"
  RESULT_VARIABLE production_cli_compare)
if(NOT production_cli_compare EQUAL 0)
  message(FATAL_ERROR "Legacy and explicit-manifest production compiler outputs differ")
endif()

# A normal .rz file is also a first-class production compiler input. Direct
# source mode must be deterministic with the one-file manifest path.
set(direct_smoke_ir "${smoke}/direct-output.fir")
execute_process(
  COMMAND "${production_executable}" "main.rz" "direct-output.fir"
  WORKING_DIRECTORY "${smoke}"
  RESULT_VARIABLE production_direct_result)
if(NOT production_direct_result EQUAL 0 OR NOT EXISTS "${direct_smoke_ir}")
  message(FATAL_ERROR "Native production compiler direct-source CLI failed with ${production_direct_result}")
endif()
execute_process(
  COMMAND "${CMAKE_COMMAND}" -E compare_files "${smoke_ir}" "${direct_smoke_ir}"
  RESULT_VARIABLE production_direct_compare)
if(NOT production_direct_compare EQUAL 0)
  message(FATAL_ERROR "Manifest and direct-source production compiler outputs differ")
endif()

# break/continue are legal only inside loops. Native production compiler must reject both
...
execute_process(
  COMMAND "${production_executable}" "invalid-break.rz" "invalid-break.fir"
  WORKING_DIRECTORY "${smoke}"
  RESULT_VARIABLE invalid_break_result)
if(invalid_break_result EQUAL 0 OR EXISTS "${smoke}/invalid-break.fir")
  message(FATAL_ERROR "Native production compiler accepted break outside a loop")
endif()
...
execute_process(
  COMMAND "${production_executable}" "invalid-continue.rz" "invalid-continue.fir"
  WORKING_DIRECTORY "${smoke}"
  RESULT_VARIABLE invalid_continue_result)
if(invalid_continue_result EQUAL 0 OR EXISTS "${smoke}/invalid-continue.fir")
  message(FATAL_ERROR "Native production compiler accepted continue outside a loop")
endif()

# Mixed i64/f64 arithmetic requires an explicit cast in the production compiler.
...
execute_process(
  COMMAND "${production_executable}" "invalid-mixed-numeric.rz" "invalid-mixed-numeric.fir"
  WORKING_DIRECTORY "${smoke}"
...
if(invalid_mixed_numeric_result EQUAL 0 OR EXISTS "${smoke}/invalid-mixed-numeric.fir")
  message(FATAL_ERROR "Native production compiler accepted mixed numeric arithmetic without an explicit cast")
endif()
...
if(NOT smoke_ir_text MATCHES "func @main\\(\\) -> i64" OR
  ...
  message(FATAL_ERROR "Native production compiler smoke Forge IR is incomplete:\n${smoke_ir_text}")
endif()
...
file(WRITE "${struct_constructor_smoke}/compiler-package.txt" "main.rz\n")
execute_process(
  COMMAND "${production_executable}" "compiler-package.txt" "struct.fir"
  WORKING_DIRECTORY "${struct_constructor_smoke}"
  RESULT_VARIABLE struct_compile_result)
if(NOT struct_compile_result EQUAL 0)
  message(FATAL_ERROR "Native production compiler rejected named struct construction with ${struct_compile_result}")
endif()
...
execute_process(
  COMMAND "${production_executable}" "main.rz" "structural-output.fir"
...
if(NOT structural_compile_result EQUAL 0 OR NOT EXISTS "${structural_ir}")
  message(FATAL_ERROR "Native production compiler structural compilation failed with ${structural_compile_result}")
endif()
if(WIN32)
...
execute_process(COMMAND "${production_executable}" "main.rz" "array-for-output.fir" WORKING_DIRECTORY "${array_for_smoke}" RESULT_VARIABLE array_for_compile_result)
if(NOT array_for_compile_result EQUAL 0 OR NOT EXISTS "${array_for_ir}")
  message(FATAL_ERROR "Native production compiler fixed-array for compilation failed with ${array_for_compile_result}")
endif()
if(WIN32)
...
execute_process(COMMAND "${production_executable}" "invalid-array-for.rz" "invalid-array-for.fir" WORKING_DIRECTORY "${array_for_smoke}" RESULT_VARIABLE invalid_array_for_result)
if(invalid_array_for_result EQUAL 0 OR EXISTS "${array_for_smoke}/invalid-array-for.fir")
  message(FATAL_ERROR "Native production compiler accepted fixed-array iteration over a scalar")
endif()
...
execute_process(COMMAND "${production_executable}" "invalid-match.rz" "invalid-match.fir" WORKING_DIRECTORY "${structural_smoke}" RESULT_VARIABLE invalid_match_result)
if(invalid_match_result EQUAL 0 OR EXISTS "${structural_smoke}/invalid-match.fir")
  message(FATAL_ERROR "Native production compiler accepted a non-exhaustive unit-enum match")
endif()
...
execute_process(COMMAND "${production_executable}" "main.rz" "payload-enum-output.fir" WORKING_DIRECTORY "${payload_enum_smoke}" RESULT_VARIABLE payload_enum_compile_result)
if(NOT payload_enum_compile_result EQUAL 0 OR NOT EXISTS "${payload_enum_ir}")
  message(FATAL_ERROR "Native production compiler payload-enum compilation failed with ${payload_enum_compile_result}")
endif()
if(WIN32)
...
execute_process(COMMAND "${production_executable}" "main.rz" "defer-output.fir" WORKING_DIRECTORY "${defer_smoke}" RESULT_VARIABLE defer_compile_result)
if(NOT defer_compile_result EQUAL 0 OR NOT EXISTS "${defer_ir}")
  message(FATAL_ERROR "Native production compiler defer compilation failed with ${defer_compile_result}")
endif()
if(WIN32)
...
execute_process(COMMAND "${production_executable}" "invalid-defer-control.rz" "invalid-defer-control.fir" WORKING_DIRECTORY "${defer_smoke}" RESULT_VARIABLE invalid_defer_result)
if(invalid_defer_result EQUAL 0 OR EXISTS "${defer_smoke}/invalid-defer-control.fir")
  message(FATAL_ERROR "Native production compiler accepted control transfer from defer")
endif()

# Multi-file/module syntax parity. The explicit manifest controls module order;
# imports are validated syntax in the compiler compiler while package
...
execute_process(COMMAND "${production_executable}" "package.txt" "module-output.fir" WORKING_DIRECTORY "${module_smoke}" RESULT_VARIABLE module_compile_result)
if(NOT module_compile_result EQUAL 0 OR NOT EXISTS "${module_ir}")
  message(FATAL_ERROR "Native production compiler multi-file/import compilation failed with ${module_compile_result}")
endif()
if(WIN32)
...
execute_process(COMMAND "${production_executable}" "invalid-import.rz" "invalid-import.fir" WORKING_DIRECTORY "${module_smoke}" RESULT_VARIABLE invalid_import_result)
if(invalid_import_result EQUAL 0 OR EXISTS "${module_smoke}/invalid-import.fir")
  message(FATAL_ERROR "Native production compiler accepted malformed import syntax")
endif()
...
execute_process(
  COMMAND "${production_executable}" "main.rz" "float-output.fir"
...
if(NOT float_compile_result EQUAL 0 OR NOT EXISTS "${float_ir}")
  message(FATAL_ERROR "Native production compiler f64 compilation failed with ${float_compile_result}")
endif()
file(READ "${float_ir}" float_ir_text)
foreach(required_float_text IN ITEMS "func @blend(%p0: f64, %p1: f64) -> f64" "const f64 1.5" "const f64 1.25e1" "mul f64" "div f64" "cmp.gt f64" "cmp.lt f64" "int_to_float.signed f64" "float_to_int.signed i64" "raz_compiler_rt_arena_get_f64" "raz_compiler_rt_arena_set_f64")
...
# Integer width/sign parity: production compiler must preserve physical widths and signedness
...
execute_process(
  COMMAND "${production_executable}" "main.rz" "integer-width-output.fir"
...
if(NOT width_compile_result EQUAL 0 OR NOT EXISTS "${width_ir}")
  message(FATAL_ERROR "Native production compiler integer-width compilation failed with ${width_compile_result}")
endif()
...
execute_process(
  COMMAND "${production_executable}" "invalid-u64-float.rz" "invalid-u64-float.fir"
  WORKING_DIRECTORY "${width_smoke}"
  RESULT_VARIABLE invalid_u64_float_result)
if(invalid_u64_float_result EQUAL 0 OR EXISTS "${width_smoke}/invalid-u64-float.fir")
  message(FATAL_ERROR "Native production compiler accepted unsupported u64-to-f64 conversion")
endif()

message(STATUS "Complete: compiler candidate emitted ${production_ir_size}-byte production compiler; production compiler compiled scalar/control-flow, f64/cast, and full integer-width parity programs; all native executables returned 42")
...
execute_process(
  COMMAND "${production_executable}" "main.rz" "slice-output.fir"
...
if(NOT slice_compile_result EQUAL 0 OR NOT EXISTS "${slice_ir}")
  message(FATAL_ERROR "Native production compiler slice compilation failed with ${slice_compile_result}")
endif()
if(WIN32)
  execute_process(
...
execute_process(
  COMMAND "${production_executable}" "invalid-shared-mutation.rz" "invalid-shared-mutation.fir"
  WORKING_DIRECTORY "${slice_smoke}"
...
if(invalid_shared_slice_result EQUAL 0 OR EXISTS "${slice_smoke}/invalid-shared-mutation.fir")
  message(FATAL_ERROR "Native production compiler accepted mutation through a shared slice")
endif()
...
# must execute in the Raz-written production compiler compiler itself.
...
execute_process(
  COMMAND "${production_executable}" "main.rz" "ownership-output.fir"
...
if(NOT ownership_compile_result EQUAL 0 OR NOT EXISTS "${ownership_ir}")
  message(FATAL_ERROR "Native production compiler ownership compilation failed with ${ownership_compile_result}")
endif()
if(WIN32)
  execute_process(
...
execute_process(
  COMMAND "${production_executable}" "invalid-use-after-move.rz" "invalid-use-after-move.fir"
  WORKING_DIRECTORY "${ownership_smoke}"
...
if(invalid_use_after_move_result EQUAL 0 OR EXISTS "${ownership_smoke}/invalid-use-after-move.fir")
  message(FATAL_ERROR "Native production compiler accepted use after move")
endif()
...
execute_process(
  COMMAND "${production_executable}" "invalid-move-while-borrowed.rz" "invalid-move-while-borrowed.fir"
  WORKING_DIRECTORY "${ownership_smoke}"
...
if(invalid_move_borrowed_result EQUAL 0 OR EXISTS "${ownership_smoke}/invalid-move-while-borrowed.fir")
  message(FATAL_ERROR "Native production compiler accepted move while borrowed")
endif()
...
execute_process(
  COMMAND "${production_executable}" "invalid-double-mut-borrow.rz" "invalid-double-mut-borrow.fir"
  WORKING_DIRECTORY "${ownership_smoke}"
...
if(invalid_double_mut_result EQUAL 0 OR EXISTS "${ownership_smoke}/invalid-double-mut-borrow.fir")
  message(FATAL_ERROR "Native production compiler accepted overlapping mutable borrows")
endif()
...
execute_process(
  COMMAND "${production_executable}" "partial-move.rz" "partial-move.fir"
  WORKING_DIRECTORY "${ownership_smoke}"
...
if(NOT partial_move_compile_result EQUAL 0 OR NOT EXISTS "${ownership_smoke}/partial-move.fir")
  message(FATAL_ERROR "Native production compiler partial-move compilation failed")
endif()
if(WIN32)
...
execute_process(
  COMMAND "${production_executable}" "disjoint-borrow.rz" "disjoint-borrow.fir"
  WORKING_DIRECTORY "${ownership_smoke}"
...
if(NOT disjoint_borrow_compile_result EQUAL 0 OR NOT EXISTS "${ownership_smoke}/disjoint-borrow.fir")
  message(FATAL_ERROR "Native production compiler disjoint-borrow compilation failed")
endif()
if(WIN32)
...
execute_process(
  COMMAND "${production_executable}" "invalid-partial-use.rz" "invalid-partial-use.fir"
  WORKING_DIRECTORY "${ownership_smoke}"
...
if(invalid_partial_use_result EQUAL 0 OR EXISTS "${ownership_smoke}/invalid-partial-use.fir")
  message(FATAL_ERROR "Native production compiler accepted use of a moved projection")
endif()
...
fn main() -> i64 {
    ...
    i64 result = raz_compiler_rt_arena_get(tracker, 0);
...
impl Drop for Resource {
    fn drop(Resource&mut self) {
        i64 count = raz_compiler_rt_arena_get(self.tracker, self.index);
        raz_compiler_rt_arena_set(self.tracker, self.index, count + 1);
    }
}
struct Pair { Resource left; Resource right; }
fn main() -> i64 {
    i64 tracker = raz_compiler_rt_arena_create(2);
    {
    ...
    i64 left = raz_compiler_rt_arena_get(tracker, 0);
    i64 right = raz_compiler_rt_arena_get(tracker, 1);
    raz_compiler_rt_arena_destroy(tracker);
    return left + right;
}
]=])
...
foreach(drop_case IN ITEMS whole partial field)
  ...
  execute_process(
    COMMAND "${production_executable}" "${drop_case}.rz" "${drop_case}.fir"
  ...
  if(NOT drop_case_compile_result EQUAL 0 OR NOT EXISTS "${drop_case_ir}")
    message(FATAL_ERROR "production compiler Drop overwrite case ${drop_case} failed to compile: ${drop_case_compile_result}")
  endif()
  if(WIN32)
...
# production compiler, not only in the C++ host frontend.
...
execute_process(
  COMMAND "${production_executable}" "main.rz" "main.fir"
...
if(NOT c1_trait_compile_result EQUAL 0 OR NOT EXISTS "${c1_trait_smoke}/main.fir")
  message(FATAL_ERROR "C1 native production compiler generic-bound/trait smoke failed to compile: ${c1_trait_compile_result}")
endif()
if(WIN32)
...
if(NOT c1_trait_codegen_result EQUAL 0 OR NOT c1_trait_link_result EQUAL 0 OR NOT EXISTS "${c1_trait_executable}")
  message(FATAL_ERROR "C1 native production compiler generic-bound/trait smoke failed to emit/link")
endif()
...
foreach(c1_invalid IN ITEMS invalid-bound invalid-associated invalid-generic-associated invalid-generic-bound invalid-overlap)
  ...
  execute_process(
    COMMAND "${production_executable}" "${c1_invalid}.rz" "${c1_invalid}.fir"
    WORKING_DIRECTORY "${c1_trait_smoke}"
