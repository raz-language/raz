...
"""Static compatibility gate for the production RXE/WebAssembly backends.
...
survived donor-backend transplants until production compiler.
"""
...
# below both driver and backend layers, so production compiler sees one ABI symbol and every
...
# Backend output paths supplied by the driver are seed arena handles, not
...
# bare.  This is intentionally a high-confidence call-site audit so production compiler
...
if failures:
    print("backend-compiler-compat: FAIL")
    for failure in failures:
        print(f"  {failure}")
    raise SystemExit(1)

print(
    "backend-compiler-compat: PASS "
