...
checks = {
    ...
    "cache validates source relocation": 'read_cache(host_build, "CMAKE_HOME_DIRECTORY", required=False)' in bootstrap and "Path(cached_source).resolve() == ROOT.resolve()" in bootstrap,
    "cache validates build relocation": 'read_cache(host_build, "CMAKE_CACHEFILE_DIR", required=False)' in bootstrap and "Path(cached_build).resolve() == host_build.resolve()" in bootstrap,
    "explicit rebuild flag exists": '"--rebuild-stage0"' in bootstrap,
    "relocated cache is regenerated": "Stage-0 cache was created in a different workspace; regenerating it for this location." in bootstrap,
