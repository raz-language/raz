...
def main() -> int:
    ...
    # CLI UX: reject command/option typos before project discovery and provide
    # concise, actionable suggestions rather than a generic manifest error.
    typo = run([args.raz, "biuld"], expect=None)
    if typo.returncode != 2:
        print("cli-output-diagnostics: FAIL: bad command did not return usage status 2", file=sys.stderr)
        return 1
    require(typo.stderr, "no such command: 'biuld'", "unknown-command error")
    require(typo.stderr, "similar command exists: 'build'", "unknown-command suggestion")
    if "raz.toml" in typo.stderr:
        print("cli-output-diagnostics: FAIL: bad command reached project discovery", file=sys.stderr)
        return 1

    bad_option = run([args.raz, "build", "--relase"], expect=None)
    if bad_option.returncode != 2:
        print("cli-output-diagnostics: FAIL: bad option did not return usage status 2", file=sys.stderr)
        return 1
    require(bad_option.stderr, "unexpected argument '--relase'", "unknown-option error")
    require(bad_option.stderr, "similar option exists: '--release'", "unknown-option suggestion")

    missing_value = run([args.raz, "build", "--profile"], expect=None)
    if missing_value.returncode != 2:
        print("cli-output-diagnostics: FAIL: missing option value did not return status 2", file=sys.stderr)
        return 1
    require(missing_value.stderr, "option '--profile' requires a value", "missing option value")

    wrong_command_option = run([args.raz, "build", "--iterations", "3"], expect=None)
    if wrong_command_option.returncode != 2:
        print("cli-output-diagnostics: FAIL: command-specific option misuse did not return status 2", file=sys.stderr)
        return 1
    require(wrong_command_option.stderr,
            "option '--iterations' is not valid for command 'build'",
            "command-specific option validation")

    command_help = run([args.raz, "build", "--help"], expect=0)
    require(command_help.stdout, "Usage:\n  raz build [path] [options]", "command help usage")
    help_alias = run([args.raz, "help", "build"], expect=0)
    require(help_alias.stdout, "Compile a package and its dependencies", "help command alias")

    doctor = run([args.raz, "doctor", "--color", "never"], expect=0)
    require(doctor.stdout, "Checking", "doctor start status")
    require(doctor.stdout, "Linker", "doctor linker check")
    require(doctor.stdout, "toolchain is ready", "doctor success summary")

    existing = work / "existing"
    existing.mkdir()
    marker = existing / "README.txt"
    marker.write_text("keep me\n", encoding="utf-8")
    initialized = run([args.raz, "init", str(existing), "--color", "never"], expect=0)
    require(initialized.stdout, "Initialized", "init status")
    if not (existing / "raz.toml").is_file() or not (existing / "src" / "main.rz").is_file():
        print("cli-output-diagnostics: FAIL: raz init did not create package files", file=sys.stderr)
        return 1
    if marker.read_text(encoding="utf-8") != "keep me\n":
        print("cli-output-diagnostics: FAIL: raz init modified an existing file", file=sys.stderr)
        return 1
    ...
    require(colored.stdout, "Compiling", "build progress")
    require(colored.stdout, "demo v0.1.0", "package build progress")
    ...
    forwarded = run([args.raz, "run", str(project), "--color", "never", "--", "hello", "--flag"], expect=0)
    require(forwarded.stdout, "Running", "run argument forwarding status")

    project_main = project / "src" / "main.rz"
    project_main.write_text("fn main() -> i64 {\n    return 7;\n}\n", encoding="utf-8")
    child_exit = run([args.raz, "run", str(project), "--force", "--color", "never"], expect=None)
    if child_exit.returncode != 7:
        print(f"cli-output-diagnostics: FAIL: raz run did not preserve child exit status (got {child_exit.returncode})", file=sys.stderr)
        return 1
    project_main.write_text("fn main() -> i64 {\n    return 0;\n}\n", encoding="utf-8")

    separator_misuse = run([args.raz, "build", str(project), "--", "hello"], expect=None)
    if separator_misuse.returncode != 2:
        print("cli-output-diagnostics: FAIL: build accepted run-only argument separator", file=sys.stderr)
        return 1
    require(separator_misuse.stderr, "argument separator '--' is only valid for command 'run'", "separator misuse")

    project_doctor = run([args.raz, "doctor", str(project), "--color", "never"], expect=0)
    require(project_doctor.stdout, "Project", "project doctor status")
    if not (project / "target" / "doctor" / "doctor.json").is_file():
        print("cli-output-diagnostics: FAIL: project doctor report missing", file=sys.stderr)
        return 1
