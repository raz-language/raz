...
CLI_HELP = (ROOT / "compiler/src/raz_driver/src/driver/cli_help.rz").read_text(encoding="utf-8")
CLI_SURFACE = CLI + "\n" + CLI_HELP
COMMANDS = (ROOT / "compiler/src/raz_driver/src/driver/commands.rz").read_text(encoding="utf-8")
LSP = (ROOT / "compiler/src/raz_driver/src/driver/lsp.rz").read_text(encoding="utf-8")
...
checks = {
    "production CLI owns lsp command": 'string candidate_lsp = "lsp"' in CLI_SURFACE and 'if (cli_arg_equals_literal(value, length, "lsp"))' in CLI_SURFACE,
    "production dispatcher routes lsp": "return lsp_command(process_argc);" in COMMANDS,
    "LSP is in production source graph": "src/raz_driver/src/driver/lsp.rz" in ORDER,
