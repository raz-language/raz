...
checks = {
    ...
    'normal compilation announces every dependency package': 'cli_print_dependency_compile_statuses(cli_command, &input, cli_manifest_path, cli_manifest_length);' in (ROOT / 'compiler/src/raz_driver/src/driver/compiler_main.rz').read_text(encoding='utf-8'),
    ...
    'update prints resolved package versions': 'cli_print_status_name_version(11, name, nl, version, vl);' in (ROOT / 'compiler/src/raz_driver/src/driver/registry.rz').read_text(encoding='utf-8'),
    'update has a completion status helper': 'fn package_registry_update_finish(i64 status)' in (ROOT / 'compiler/src/raz_driver/src/driver/registry.rz').read_text(encoding='utf-8'),
    'build has a linking status': 'cli_print_status(12, cli_manifest_path, cli_manifest_length);' in (ROOT / 'compiler/src/raz_driver/src/driver/compiler_main.rz').read_text(encoding='utf-8'),
}
