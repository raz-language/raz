...
checks = {
    ...
    'component host implements browser ABI': all(x in web_codegen_source(ROOT) for x in ['dom_value_length:', 'session_storage_set:', 'location_origin_length:', 'url_query_value_length:', 'timer_set_interval:']),
    'static-page host implements browser ABI': all(x in (ROOT/'library/web/src/client_host.rz').read_text() for x in ['dom_value_length:', 'session_storage_set:', 'location_origin_length:', 'url_query_value_length:', 'timer_set_interval:']),
}
