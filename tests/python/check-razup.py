...
source = (ROOT / 'compiler/src/raz_driver/src/driver/razup.rz').read_text(encoding='utf-8')
main = (ROOT / 'compiler/src/raz_driver/src/driver/compiler_main.rz').read_text(encoding='utf-8')
host = (ROOT / 'compiler/src/raz_driver/src/driver/host_support.rz').read_text(encoding='utf-8')
...
if 'src/raz_driver/src/driver/razup.rz' not in order or order[-1] != 'src/main.rz':
    raise SystemExit('razup: canonical semantic source set is incomplete')
if order.index('src/raz_driver/src/driver/razup.rz') > order.index('src/main.rz'):
