...
checks = {
    'bench command is recognized': 'cli_arg_equals_literal(value, length, "bench")' in cli and 'return 40;' in cli,
    'cache command is recognized': 'cli_arg_equals_literal(value, length, "cache")' in cli and 'return 41;' in cli,
    'bench has configurable iterations': 'fn cli_bench_iterations_option' in cli and 'string prefix = "--iterations=";' in cli,
    ...
    'clean removes legacy pre-R19 state': 'string legacy = ".raz";' in cli and 'raz_compiler_rt_remove_path_ascii(path, 4)' in cli,
    'clean removes build state': 'raz_compiler_rt_remove_path_ascii(path, 5)' in cli,
    'Stage-0 does not duplicate production clean': 'clean' not in host_main and 'cli_clean_project' not in host_main,
