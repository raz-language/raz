...
def main():
    ...
    if ns.compiler: compiler=str(Path(ns.compiler).resolve())
        ...
        compiler=str(project/'target'/'host'/'debug'/('raz-compiler.exe' if os.name=='nt' else 'raz-compiler'))
    ...
    assert str(store).replace('\\','/') in (app/'raz.toml').read_text().replace('\\','/')
    ...
    before=(app/'raz.lock').read_bytes();run([compiler,'update'],app,offline);assert (app/'raz.lock').read_bytes()==before
