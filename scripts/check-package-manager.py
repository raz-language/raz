...
def main():
    ...
    root=Path(ns.root).resolve(); work=Path(ns.work).resolve(); shutil.rmtree(work,ignore_errors=True); project=work/'compiler'; shutil.copytree(root/'compiler',project)
    env=os.environ.copy();env['RAZ_LINKER']=ns.linker
    ...
    run([str(compiler),'remove','missing'],rootpkg,env,expect=59)
