...
def main() -> int:
    ...
    if not compiler.is_file():
        raise RuntimeError(f'missing self-host compiler: {compiler}')

    # These are production-compiler semantic gates. They intentionally run only
    # through the Raz-written compiler; Stage 0 is frozen bootstrap machinery.
    run([str(compiler), '--check', str(root / 'tests/traits/nested_associated_normalization.rz')], cwd=work, env=env)
    run([str(compiler), '--check', str(root / 'examples/traits/trait_alias_generic_impl.rz')], cwd=work, env=env)
    run([str(compiler), '--check', str(root / 'examples/traits/invalid_overlapping_generic_impl.rz')], cwd=work, env=env, should_fail=True)

    print('selfhost-trait-solver: PASS')
    ...
    print('  Stage 0: not used as a feature-parity oracle')
    return 0
