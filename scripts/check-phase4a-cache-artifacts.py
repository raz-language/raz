...
checks = {
    ...
    'cache arenas are initialized and destroyed with HIR':
        'out.incremental_module_hir_fingerprints = raz_rt_stage1_arena_create(4096);' in model and
        'raz_rt_stage1_arena_destroy(out.incremental_module_hir_fingerprints);' in model and
        'raz_rt_stage1_arena_destroy(out.incremental_module_cached_mir_fingerprints);' in model and
        'raz_rt_stage1_arena_destroy(out.incremental_module_cache_kinds);' in model,
}

failed = [name for name, ok in checks.items() if not ok]
if failed:
    for name in failed:
        print(f'phase4a-cache-artifacts: FAIL: {name}')
    sys.exit(1)

print('phase4a-cache-artifacts: PASS')
print('  module HIR identity is persisted explicitly')
print('  optimized MIR cache identity is exact and schema-versioned')
print('  cache-hit state survives into the self-hosted driver')
