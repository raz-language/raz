...
checks = {
    ...
    'borrow expressions emit projection-aware events': all(x in lowering for x in [
        'mir_record_hir_borrow_event', 'raz_rt_stage1_arena_get(hir.node_values, node)']),
    ...
    'CFG joins conservatively union active loans': all(x in loans for x in [
        ...
        'raz_rt_stage1_arena_get(cfg.successor_a, predecessor) == block']),
...
if failed:
    for name in failed:
        print(f'mir2g-borrow-regions: FAIL: {name}')
    sys.exit(1)

print('mir2g-borrow-regions: PASS')
