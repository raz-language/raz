...
def _resolve_executable(candidate: str, env: dict[str, str]) -> str | None:
    """Resolve a compiler/tool exactly as the legacy PowerShell bootstrap did."""
    if not candidate:
        return None
    path = Path(candidate)
    if path.is_file():
...
def _test_cxx20_toolchain(compiler: str, scratch: Path, env: dict[str, str]) -> tuple[bool, str]:
    """Compile the same C++20 preflight used by scripts/bootstrap.ps1."""
...
def source_order() -> list[str]:
    order = []
    for raw in (ROOT / "compiler" / "bootstrap-source-order.txt").read_text(encoding="utf-8").splitlines():
    ...
    if not order or order[-1] != "src/main.rz":
        raise RuntimeError("compiler/bootstrap-source-order.txt must be non-empty and end with src/main.rz.")
    return order


def prepare_recursive_stage(stage_dir: Path, order: list[str]) -> None:
    if stage_dir.exists():
        shutil.rmtree(stage_dir)
    # Stage 1+ understands semantic namespaces/imports. Recursive stages compile
    # the canonical compiler project; bootstrap-source-order.txt is used only as
    ...
    shutil.copytree(ROOT / "compiler", stage_dir, ignore=shutil.ignore_patterns(".raz", "target"))
    (stage_dir / "source-order.txt").write_text("\n".join(order) + "\n", encoding="ascii")


def invoke_selfhost(label: str, compiler: Path, stage_dir: Path, args: list[str], interval: int) -> None:
    print(f"[RUN] {label}\n      {compiler} {' '.join(args)}", flush=True)
    diagnostic = stage_dir / "stage1-diagnostic.txt"
    diagnostic.unlink(missing_ok=True)
    proc = subprocess.Popen([str(compiler), *args], cwd=stage_dir)
    ...
    while proc.poll() is None:
        ...
        if raw and raw != last_diag:
            last_diag = raw
            print(f"      [{time.monotonic() - started:6.1f}s] self-host diagnostic: {raw}", flush=True)
...
def main() -> int:
    ...
    parser.add_argument("--host-preset", "-HostPreset", choices=("dev", "release"), default="release")
    parser.add_argument("--jobs", "-Jobs", type=int, default=max(1, os.cpu_count() or 1))
    parser.add_argument("--status-interval", "-StageStatusIntervalSeconds", type=int, default=15)
    ...
    stages = ROOT / "build" / ("windows-all-stages" if IS_WINDOWS else "all-stages")
    if args.clean:
        shutil.rmtree(host_build, ignore_errors=True)
        shutil.rmtree(stages, ignore_errors=True)
    stages.mkdir(parents=True, exist_ok=True)

    banner(f"Stage 0 - configure and build native host toolchain ({platform.system()})")
    ...
    run("Build host compiler/runtime/Forge", [cmake, "--build", "--preset", args.host_preset, "--parallel", str(args.jobs), "--target", "raz0", "razc0", "raz_runtime", "raz_forge_bridge", "forge"], env=env)
    compiler = read_cache(host_build, "CMAKE_CXX_COMPILER")
    env["RAZ_LINKER"] = compiler

    raz0 = find_artifact(host_build, [f"raz0{EXE}"])
    ...
    # building it must be repeated at the final recursive-stage link boundary.
    ...
    if openssl_enabled and len(runtime_deps) < 2:
        raise RuntimeError(
            ...
            "attempting a broken recursive-stage link."
        )
    ...
    order = source_order()
    print(f"Canonical self-host compiler: {len(order)} Raz modules")

    banner("Stage 1 - build Raz compiler with Stage 0")
    stage1_project = stages / "stage1-project"
    shutil.rmtree(stage1_project, ignore_errors=True)
    shutil.copytree(ROOT / "compiler", stage1_project, ignore=shutil.ignore_patterns(".raz", "target"))
    ...
    #
    # RXE and WebAssembly are canonical self-hosted backends, but Stage 1 only
    # needs Forge/LLVM to produce Stage 2. Keeping the optional backends out of
    # this one disposable seed prevents the frozen native Stage 0 frontend from
    # crossing its peak-memory ceiling as the Raz-owned compiler grows. Stage 2
    # and every recursive stage below compile the complete canonical source tree.
    for optional_backend in ("wasm", "rxe"):
        shutil.rmtree(stage1_project / "src" / "backend" / optional_backend, ignore_errors=True)
    legacy_order = [
        item for item in (stage1_project / "bootstrap-source-order.txt").read_text(encoding="utf-8").splitlines()
        if not item.startswith("src/backend/wasm/") and not item.startswith("src/backend/rxe/")
    ]
    legacy_backend = stage1_project / "src" / "driver" / "backend.rz"
    backend_text = legacy_backend.read_text(encoding="utf-8")
    ...
    if wasm_options >= 0:
        ...
        if option_end < 0:
            raise RuntimeError("Could not prepare Stage 1 backend option view.")
    ...
    legacy_backend.write_text(backend_text, encoding="utf-8")
    for source in (stage1_project / "src").rglob("*.rz"):
        ...
        source.write_text("\n".join(lines) + "\n", encoding="utf-8")
    (stage1_project / "source-order.txt").write_text("\n".join(legacy_order) + "\n", encoding="ascii")
    run("Stage 0 -> Stage 1", [str(raz0), "build", str(stage1_project), "--target", "host", "--profile", args.bootstrap_profile, "--force"], env=env)
    built = stage1_project / "target" / "host" / args.bootstrap_profile / f"raz-compiler{EXE}"
    if not built.is_file():
        raise RuntimeError(f"Stage 1 compiler was not produced: {built}")
    stage1_dir = stages / "stage1"
    stage1_dir.mkdir(parents=True, exist_ok=True)
    stage1 = stage1_dir / f"raz-stage1{EXE}"
    shutil.copy2(built, stage1)
    if not IS_WINDOWS:
    ...
    previous = stage1
    for stage in (2, 3, 4):
        banner(f"Stage {stage} - recursive self-host compilation")
        directory = stages / f"stage{stage}"
        prepare_recursive_stage(directory, order)
        obj = directory / f"stage{stage}{OBJ}"
        invoke_selfhost(
            f"Stage {stage - 1} -> Stage {stage}",
            previous,
            directory,
            [
                "build",
        ...
        if not obj.is_file() or obj.stat().st_size < 500_000:
            raise RuntimeError(f"Stage {stage} native object is missing or unexpectedly small: {obj}")
        exe = directory / f"raz-stage{stage}{EXE}"
        link_stage(compiler, obj, runtime, bridge, forge, exe, env, runtime_deps)
        run(f"Smoke-test Stage {stage}", [str(exe), "--version"], cwd=directory, env=env)
        generated.append((stage, obj, exe, digest(obj)))
        previous = exe

    banner("Verify deterministic recursive fixed point")
    sizes = {item[1].stat().st_size for item in generated}
    hashes = {item[3] for item in generated}
    if len(sizes) != 1 or len(hashes) != 1:
        raise RuntimeError("Recursive fixed point failed: Stage 2/3/4 native objects differ.")
    fixed_hash = generated[0][3]
    fixed_size = generated[0][1].stat().st_size
    print(f"Stage 2 == Stage 3 == Stage 4 ({fixed_size} object bytes)\nSHA-256: {fixed_hash}")

    if args.run_tests:
        banner("Run CTest qualification")
        run("CTest", [ctest, "--test-dir", str(host_build), "--output-on-failure", "-j", str(args.jobs)], env=env)

    summary = stages / "BUILD-SUMMARY.txt"
    summary.write_text(
        "Raz portable all-stage bootstrap succeeded.\n\n"
        f"Platform: {platform.platform()}\nHost preset: {args.host_preset}\nBootstrap profile: {args.bootstrap_profile}\nC++ compiler: {compiler}\n\n"
        + "\n".join(f"Stage {stage}: {exe}" for stage, _, exe, _ in generated)
        + f"\n\nFixed-point object bytes: {fixed_size}\nFixed-point SHA-256: {fixed_hash}\nStage 2 == Stage 3 == Stage 4: yes\n",
        encoding="utf-8",
    )
    banner("ALL STAGES BUILT SUCCESSFULLY")
    print(f"Artifacts: {stages}\nSummary  : {summary}")
    return 0
