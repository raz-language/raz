...
assert len(ops)==68, f'freeze expects 68 ISA v1 opcodes, found {len(ops)}'
...
subprocess.run([sys.executable,str(root/'scripts/generate-rxe-freeze.py')],check=True,cwd=root,stdout=subprocess.DEVNULL)
manifest=(root/'docs/RXE-ISA-v1-FREEZE.md').read_text()
...
print('rxe-freeze-gate: PASS (ISA v1 semantic manifest + independent binary semantic checks)')
