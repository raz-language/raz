...
"""Execute maintained Raz standard-library smokes through Forge and LLVM.

The cases compose the real library modules with tiny test drivers, so this
qualifies Raz-owned library algorithms and their true OS/runtime boundaries.
"""
...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="raz-stdlib-exec-") as directory:
        ...
        for case in CASES:
            ...
            llvm_ir = work / f"{case}.ll"
            llvm_exe = work / (f"{case}-llvm.exe" if os.name == "nt" else f"{case}-llvm")

            result = run([str(args.razc), "--backend=forge", str(source), str(fir)], timeout=args.timeout)
            if result.returncode != 0:
                fail(f"{case} Forge compile", result)
            if case == "symbol_namespace":
                text = fir.read_text(encoding="utf-8")
                for name in ("close", "read", "write"):
                    if f"@__raz_fn_{name}" not in text or f"func @{name}(" in text:
                        raise SystemExit(f"stdlib-execution: FAIL {case} Forge symbol namespace for {name}")
                for name in ("errno", "stdin"):
                    if f"@__raz_global_{name}" not in text:
                        raise SystemExit(f"stdlib-execution: FAIL {case} Forge global namespace for {name}")
            ...
            link_cmd = [str(linker), str(forge_object), str(args.runtime), *runtime_link_deps]
            if os.name != "nt":
            ...
            result = run([str(args.razc), "--backend=llvm", str(source), str(llvm_ir)], timeout=args.timeout)
            if result.returncode != 0:
                fail(f"{case} LLVM compile", result)
            if case == "symbol_namespace":
                text = llvm_ir.read_text(encoding="utf-8")
                for name in ("close", "read", "write"):
                    if f"@__raz_fn_{name}" not in text or f"define i64 @{name}(" in text:
                        raise SystemExit(f"stdlib-execution: FAIL {case} LLVM symbol namespace for {name}")
                for name in ("errno", "stdin"):
                    if f"@__raz_global_{name}" not in text:
                        raise SystemExit(f"stdlib-execution: FAIL {case} LLVM global namespace for {name}")
            llvm_link = [str(linker), "-Wno-override-module", "-x", "ir", str(llvm_ir), "-x", "none", str(args.runtime), *runtime_link_deps]
            if os.name != "nt":
                llvm_link += ["-lpthread", "-ldl"]
            llvm_link += ["-o", str(llvm_exe)]
            result = run(llvm_link, timeout=args.timeout)
            if result.returncode != 0:
                fail(f"{case} LLVM link", result)
            result = run([str(llvm_exe)], timeout=args.timeout)
            if result.returncode != 0:
                fail(f"{case} LLVM run rc={result.returncode}", result)

            print(f"stdlib-execution: PASS {case} (Forge=0 LLVM=0)")
