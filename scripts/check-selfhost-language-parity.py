...
Runs every single-file `razc0 --check` conformance case registered in
`tests/CMakeLists.txt` through both the native Stage 0 compiler and a generated
...
def discover_cases(root: Path) -> list[tuple[str, str]]:
    ...
    pattern = re.compile(
        r"add_test\(NAME\s+([\w-]+)\s+COMMAND\s+razc0\s+--check\s+"
...
def main() -> int:
    ...
    parser.add_argument("--native", required=True, type=Path, help="razc0 executable")
    parser.add_argument("--selfhost", required=True, type=Path, help="generated Raz compiler executable")
    ...
    selfhost = args.selfhost.resolve()
    ...
    for test_name, relative in cases:
        ...
        selfhost_rc, selfhost_timeout, selfhost_detail = run(
            [str(selfhost), "--check", str(source)], root, args.timeout
        )
        native_ok = native_rc == 0 and not native_timeout
        selfhost_ok = selfhost_rc == 0 and not selfhost_timeout
        match = native_ok == selfhost_ok and not native_timeout and not selfhost_timeout
        results.append(
            {
                "test": test_name,
                "source": relative,
                "match": match,
                "native_ok": native_ok,
                "selfhost_ok": selfhost_ok,
                "native_rc": native_rc,
                "selfhost_rc": selfhost_rc,
                "native_timeout": native_timeout,
                "selfhost_timeout": selfhost_timeout,
                "native_detail": native_detail[-1200:],
                "selfhost_detail": selfhost_detail[-1200:],
            }
        )
    ...
    summary = {
        ...
        "selfhost_accept": sum(result["selfhost_ok"] for result in results),
        "selfhost_timeouts": sum(result["selfhost_timeout"] for result in results),
    }

    print(
        "Raz native/self-host --check parity: "
        f"{summary['matches']}/{summary['total']} matched; "
        f"{summary['mismatches']} mismatch(es); "
        f"{summary['selfhost_timeouts']} self-host timeout(s)"
    )
    for result in mismatches:
        print(
            f"MISMATCH {result['source']}: native={result['native_rc']} "
            f"selfhost={result['selfhost_rc']}"
            + (" [self-host timeout]" if result["selfhost_timeout"] else "")
        )
