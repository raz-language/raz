...
if violations:
    print('Native C/C++ files must remain inside src/bootstrap, src/forge, src/runtime, or tests/native:')
...
try:
    ...
    print(f'Canonical self-host compiler source set is invalid: {error}')
    sys.exit(1)
if canonical_sources[-1] != root / 'compiler' / 'src' / 'main.rz':
    print('Canonical self-host compiler entrypoint must be compiler/src/main.rz and last in bootstrap-source-order.txt')
    sys.exit(1)

forbidden = [
    root / 'frontend.rz',
    root / 'compiler' / 'frontend.rz',
    root / 'bootstrap' / 'stage1' / 'frontend.rz',
    root / 'bootstrap' / 'stage1' / 'compiler' / 'frontend.rz',
    root / 'bootstrap' / 'stage1' / 'src' / 'main.rz',
]
found = [str(path.relative_to(root)) for path in forbidden if path.exists()]
if found:
    print('Self-host compiler source layout violation:')
    for item in found:
...
print(f'self-host source layout: PASS ({len(canonical_sources)} ordered Raz modules)')
