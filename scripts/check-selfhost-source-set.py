...
try:
    ...
    print(f"selfhost-source-set: FAIL: {error}")
    sys.exit(1)
...
if set(discovered) != set(path.resolve() for path in ordered):
    ...
    print("selfhost-source-set: FAIL: bootstrap-source-order.txt does not cover compiler/src exactly")
...
if relative[-1] != "src/main.rz":
    print("selfhost-source-set: FAIL: src/main.rz must be the final compiler source")
    sys.exit(1)

# Physical filenames are descriptive; bootstrap seed ordering belongs in
# bootstrap-source-order.txt rather than numeric filename prefixes. Production
...
for entry in relative:
    ...
    if re.match(r"^[0-9]+_", name):
        print(f"selfhost-source-set: FAIL: compiler module uses numeric ordering prefix: {entry}")
        sys.exit(1)

main_text = ordered[-1].read_text(encoding="utf-8")
main_defs = re.findall(r"(?m)^fn\s+main\s*\(", main_text)
if len(main_defs) != 1:
    print("selfhost-source-set: FAIL: src/main.rz must define exactly one main function")
    sys.exit(1)
for path in ordered[:-1]:
    if re.search(r"(?m)^fn\s+main\s*\(", path.read_text(encoding="utf-8")):
        print(f"selfhost-source-set: FAIL: non-entry module defines main: {path.relative_to(root)}")
        sys.exit(1)

line_counts = [(path, len(path.read_text(encoding="utf-8").splitlines())) for path in ordered]
if line_counts[-1][1] > 750:
    print(f"selfhost-source-set: FAIL: entrypoint regressed into a monolith ({line_counts[-1][1]} lines)")
    sys.exit(1)
largest_path, largest_lines = max(line_counts, key=lambda item: item[1])
if largest_lines > 5000:
    print(f"selfhost-source-set: FAIL: compiler module too large: {largest_path.relative_to(root)} ({largest_lines} lines)")
    sys.exit(1)

print(
    f"selfhost-source-set: PASS ({len(ordered)} modules, "
